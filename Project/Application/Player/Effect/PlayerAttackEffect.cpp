...
void PlayerAttackEffect::Update(const Matrix4x4& cameraMatrix, const Matrix4x4& viewMatrix) {
    ...
    if (hitTimer_ <= 0.2f) {
        ...
        worldTransform_.transform_.scale = Lerp(Vector3(0.0f, 0.0f, 0.0f), Vector3(endScale_, endScale_, endScale_), localT);
    } else {
...
void PlayerAttackEffect::Emitter(const Vector3& pos) {
    if (!isActive_) {
        isActive_ = true;
        hitTimer_ = 0.0f;
        emitPos_ = pos;
