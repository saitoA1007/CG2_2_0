...
void ScanLinePSO::Initialize(ID3D12Device* device, DXC* dxc, LogManager* logManager) {
    ...
    vsBlob = dxc->CompileShader(L"Resources/Shaders/PostEffect/ScanLine/ScanLine.VS.hlsl",
        L"vs_6_0", dxc->dxcUtils_.Get(), dxc->dxcCompiler_.Get(), dxc->includeHandler_.Get());
    assert(vsBlob != nullptr);

    Microsoft::WRL::ComPtr<IDxcBlob> psBlob;
    // 明るい部分を抽出する用
    psBlob = dxc->CompileShader(L"Resources/Shaders/PostEffect/ScanLine/ScanLine.PS.hlsl",
    ...
    // 書き込むためのアドレスを取得
    parameterResource_->Map(0, nullptr, reinterpret_cast<void**>(&scanLineData_));
    scanLineData_->interval = 96.0f;
    scanLineData_->speed = -2.0f;
    scanLineData_->time = 0.0f;
    scanLineData_->lineColor = { 0.3f,0.3f,0.3f };
}

void ScanLinePSO::Draw(ID3D12GraphicsCommandList* commandList, D3D12_GPU_DESCRIPTOR_HANDLE inputSRV) {
