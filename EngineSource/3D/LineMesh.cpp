...
void LineMesh::Initialize(const Vector3& p1, const Vector3& p2, const Vector4& color) {
	...
	// 頂点バッファを作成
	// vertexResourceを作成
	vertexResource_ = CreateBufferResource(device_, sizeof(VertexPosColor) * 4);
	// リソースの先頭のアドレスから使う
	vertexBufferView_.BufferLocation = vertexResource_->GetGPUVirtualAddress();
	// 使用するリソースのサイズは頂点4つ分(線を描画させる性質上先端が1ピクセル未満になってしまうのを回避するため)
	vertexBufferView_.SizeInBytes = sizeof(VertexPosColor) * 4;
	// 1頂点あたりのサイズ
	...
	// 2つの頂点テータを作成
	vertexPosColor_[0].pos = p1;
	vertexPosColor_[0].color = color;
	vertexPosColor_[1].pos = p2;
	vertexPosColor_[1].color = color;
	vertexPosColor_[2].pos = p2;
	vertexPosColor_[2].color = color;
	vertexPosColor_[3].pos = p1;
	vertexPosColor_[3].color = color;
}
